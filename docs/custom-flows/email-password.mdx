...
4. If the email address verification is successful, complete the sign-up process by creating the user account and setting their session as active.
    - When using the iOS SDK, once the sign up process completes, you will have a `Session` and a `User` available on the `Clerk` shared instance.

<Tabs type="framework" items={["Next.js","React","Remix", "Gatsby", "Expo", "JavaScript", "iOS (Swift)"]}>
  <Tab>
